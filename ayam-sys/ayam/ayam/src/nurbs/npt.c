 ...
 ay_point *nselp = NULL, *p = NULL;
  ...
  switch(mode)
      ...
      for(j = 0; j < np->height; j++)
	{
      ...
      if(np->utype)
	{
	  ay_status = ay_npt_closeu(np, np->utype);
	}
      ...
      for(j = 0; j < np->width; j++)
	{
      ...
      if(np->vtype)
	{
	  ay_status = ay_npt_closev(np, np->vtype);
	}

      break;
    case 2:
      ay_npt_fair(np, selp, tol, 0, worst);
      ay_npt_fair(np, selp, tol, 1, worst);
      break;
    case 3:
      ay_npt_fair(np, selp, tol, 1, worst);
      ay_npt_fair(np, selp, tol, 0, worst);
		  ...
		  if(flash)
		      ...
		      if(drag)
			{
			  ...
			  if(rem)
				...
				ay_npt_selectbound(o, pid, bid, AY_TRUE);
			    }
			}
		      else
			{
