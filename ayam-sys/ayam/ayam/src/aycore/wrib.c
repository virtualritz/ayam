...
unsigned int ay_wrib_primlevel = 0;
 ...
 */
void
ay_wrib_rioptions(int searchpathsonly)
{
 ay_riopt *riopt = NULL;
...
ay_wrib_displaytags(int have_ridisplay)
{
 ...
 RtInt argc;
  ...
  while(tag)
    {
      if(tag->type == ay_ridisp_tagtype && tag->val)
	{
	  name = NULL;
	  val = tag->val;
	  if(val)
...
ay_wrib_hidertags(void)
{
 ...
 RtInt argc;
  ...
  while(tag)
    {
      if(tag->type == ay_rihider_tagtype && tag->val)
	{
	  val = tag->val;
	  len = strlen(val)+1;
	  if(len > 1)
  ...
  */
  ...
  if(ay_prefs.use_sm == 1)
      ...
      if(ay_status)
	{
      ...
      ay_sm_wriballsm(file, objfile, ay_root->next, NULL,
		      width, height, AY_FALSE);
  ...
  /* write search paths from root RiOptions */
  ay_wrib_rioptions(AY_TRUE);
...
ay_wrib_sm(char *file, int width, int height, int selonly)
{
 ...
 if(!ay_prefs.resolveinstances)
  ...
  ay_sm_wriballsm(file, objfile, ay_root->next, NULL, width, height, selonly);
	  ...
	  if(selonly)
	    {
	      ...
	      RiBegin(filename);
	       while(sel)
		 {
		   ay_status = ay_wrib_object(filename, sel->object);
		   if(ay_status)
		     break;
		   sel = sel->next;
		 } /* while */
	  ...
	  else
	    {
	      ...
	      RiBegin(filename);
	       while(o)
		 {
		   ay_status = ay_wrib_object(filename, o);
		   if(ay_status)
		     break;
		   o = o->next;
		 } /* while */
