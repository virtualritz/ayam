...
/* ay_instance_peekcb:
 *  peek callback function of instance object
 */
int
ay_instance_peekcb(ay_object *o, unsigned int type, ay_object **result,
		   double *transform)
{
 int ay_status = AY_OK;
 ay_object *m = NULL;
 ...
 ay_providecb *provcb = NULL;
  ...
  if(!result)
	...
	{
	  return ay_peek_object(m, type, NULL, NULL);
	}
    }
  ...
  if(m->type == type)
	...
	from the instanced object instead
      */
      /* use the peek callback directly (and not simply ay_peek_object())
	 to avoid that the trafos of m (which the instance replaces)
	 end up in the transform array */
      arr = ay_peekcbt.arr;
      cb = (ay_peekcb *)(arr[m->type]);
      if(cb)
	{
	  ay_status = cb(m, type, result, transform);
	}
      else
	{
	  arr = ay_providecbt.arr;
	  provcb = (ay_providecb *)(arr[m->type]);
	  if(provcb)
	    {
	      ay_status = provcb(m, UINT_MAX-type, result);
	    }
	}
    } /* if */
 ...
 */
int
ay_instance_drawacb(struct Togl *togl, ay_object *o)
{
...
ay_instance_init(Tcl_Interp *interp)
{
 int ay_status = AY_OK;
  ...
  ay_status += ay_peek_register(ay_instance_peekcb, AY_IDINSTANCE);

  ay_status += ay_draw_registerdacb(ay_instance_drawacb, AY_IDINSTANCE);
