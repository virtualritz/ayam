...
proc shortcut_viewactions { w } {
 global ay ayviewshortcuts
    ...
    # Roll by mouse wheel
    # XXXX ToDo: should we bind B4/B5 on Win32/Aqua?
    bind $w.f3D.togl <${ayviewshortcuts(RollMod)}-ButtonPress-4> {viewRoll %W %D;break}

    bind $w.f3D.togl <${ayviewshortcuts(RollMod)}-ButtonPress-5> {viewRoll %W -%D;break}

    if { $ay(ws) == "Win32" || $ay(ws) == "Aqua" } {
	if { [string first ".view" $w] != 0 } {
	    # internal view
	    if { $ay(ws) == "Win32" } {
		bind $w <${ayviewshortcuts(RollMod)}-MouseWheel> {viewRoll %W.f3D.togl %D;break}
	    } else {
		# Aqua
		bind $w.f3D.togl <${ayviewshortcuts(RollMod)}-MouseWheel> {viewRoll %W %D;break}
	    }
	} else {
	    # external view
	    bind $w <${ayviewshortcuts(RollMod)}-MouseWheel> {viewRoll %W %D;break}
	}
	# if internal or external
    }
    # if Win32 or Aqua
