...
ay_matt_wrib(ay_mat_object *m)
{
 ...
 char *str = NULL;
  ...
  if(m->objptr)
    {
      ay_status = ay_riattr_wrib(m->objptr);
      if(ay_status)
	return ay_status;
      ay_status = ay_tc_wrib(m->objptr);
      if(ay_status)
	return ay_status;
    }
...
ay_matt_nomaterial(unsigned int type)
{
 ...
 int new_item = 0;
  ...
  entry = Tcl_CreateHashEntry(ht, (char*)ay_otype_getpointer(type), &new_item);
...
{
  if(Tcl_FindHashEntry(&ay_matt_blacklist_ht,
		       (char *)ay_otype_getpointer(type)) != NULL)
    return AY_FALSE;
  else
    return AY_TRUE;
} /* ay_matt_mayhavematerial */
...
ay_matt_init(Tcl_Interp *interp)
{
  ...
  (void)ay_tags_register(ay_matt_mitagname, &ay_matt_mitagtype);
